...
def calc_IST_by_split_window(bt1_array, bt2_array, senz_array, param):
    ...
    get_IST = (
        ...
        + p["b"] * bt1
        + p["c"] * (bt1 - bt2)
        + p["d"] * (bt1 - bt2) * (1.0 / np.cos(np.deg2rad(senz)) - 1.0)
    )
    outArray = np.where(
        bt1_array < 240,
        get_IST(bt1_array, bt2_array, senz_array, param["240K"]),
        np.where(
            (bt1_array >= 240) & (bt1_array < 260),
            get_IST(bt1_array, bt2_array, senz_array, param["240-260K"]),
