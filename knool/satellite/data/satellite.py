...
class TLEsPerSat:
    def __init__(self, infile):
        ...
        for i, tle in enumerate(self.tles):
            ...
            num_list.append(i)
        self.df0 = pd.DataFrame(num_list, columns=["num"]).assign(
            Date=pd.to_datetime(date_list)).set_index("Date")
        self.df = self.df0
    ...
    def calc_positions_between(self, fdate, ldate, interval):
        ...
        ds_date = pd.date_range(start=fdate, end=ldate,
                                freq=str(interval) + "min", tz=utc)
        pos_list = []
        date_list = []
        for date in ds_date:
            print(date)
    ...
    def calc_positions_faster_between(self, fdt, ldt, interval):
        ...
        df = self.df
        row1 = df.loc[df.index == df.index.unique(
        )[df.index.unique().get_indexer([pd.to_datetime(mdt)], method="nearest")[0]]]
        num = row1.num.values[0]

        date_ds = pd.date_range(
            start=fdt, end=ldt, freq=str(interval) + "min", tz=utc)
    ...
    def calc_buff_positions_between(self, fdt, ldt, interval, distance, ori="right"):
        ...
        for i, date in enumerate(ds_date):
            ...
            lat3, lon3, ele3 = geo_info.calc_line_buffer_point(
                lat, lon, 0, lat2, lon2, 0, distance, ori)
    ...
    def calc_buff_positions_faster_between(self, fdt, ldt, interval, distance, ori="right"):
        ...
        date_ds = pd.date_range(
            start=fdt, end=ldt + timedelta(minutes=interval), freq=str(interval) + "min", tz=utc)
        date_list = date_ds.to_list()
        date_sf = load.timescale().from_datetimes(date_list)

        lat, lon, _ = self._get_position(num, date_sf)

        lat2, lon2, _ = geo_info.calc_line_buffer_point(
            lat[0:-1], lon[0:-1], 0, lat[1:], lon[1:], 0, distance, ori)
    ...
    def _calc_buff_positions__left_right_between(self, fdate, ldate, interval, distance, ori="middle"):
        ...
        if ori == "middle":
            rpos_array = self.calc_buff_positions_faster_between(
                fdate, ldate, interval, distance / 2, ori="right")
            lpos_array = self.calc_buff_positions_faster_between(
                fdate, ldate, interval, distance / 2, ori="left")
        elif ori == "right":
            rpos_array = self.calc_buff_positions_faster_between(
                fdate, ldate, interval, distance, ori="right")
            lpos_array = self.calc_positions_faster_between(
                fdate, ldate, interval)
        elif ori == "left":
            lpos_array = self.calc_buff_positions_faster_between(
                fdate, ldate, interval, distance, ori="left")
            rpos_array = self.calc_positions_faster_between(
                fdate, ldate, interval)
        else:
    ...
    def calc_buff_area_between(self, fdate, ldate, interval, distance, ori="middle"):
        lpos_array, rpos_array = self._calc_buff_positions__left_right_between(
            fdate, ldate, interval, distance, ori)
    ...
    def calc_scene_areas(self, fdate, ldate, interval, distance, pinterval, ori="middle"):
        ...
        lpos_sub = np.lib.index_tricks.as_strided(
            lpos_array.copy(), (row_num, pinterval, 1, col_num), (bit *
                                                                  col_num * (pinterval - 1), bit * col_num, 8, 8)
        )[0:pol_num]
        rpos_sub = np.lib.index_tricks.as_strided(
            rpos_array.copy(), (row_num, pinterval, 1, col_num), (bit *
                                                                  col_num * (pinterval - 1), bit * col_num, 8, 8)
        )[0:pol_num]
        rpos_sub2 = rpos_sub[:, ::-1, :, :]
        outlines_sub = np.concatenate(
            [lpos_sub, rpos_sub2], axis=1)[:, :, 0, :]

        self.output = outlines_sub
        date_list = self.output_attribute
        stime_list = [date_obj for date_obj in date_list[: -
                                                         (pinterval - 1): (pinterval - 1)]]
        ftime_list = [date_obj for date_obj in date_list[(
            pinterval - 1):: (pinterval - 1)]]
        self.output_attribute = {
            "StartTime": stime_list, "EndTime": ftime_list}
    ...
    def calc_intersect_scene_areas(
        ...
        for geom, trans in zip(geom1_list, trans_list):
            ...
            attr2_dict = {}
            for key in keys:
                attr2_dict[key] = [attr for (attr, bool) in zip(
                    self.output_attribute[key], bool_list) if bool]
            geom2_llist.append(geom2_list)
            attr2_dlist.append(attr2_dict)

        [geom1.Transform(trans)
         for (geom1, trans) in zip(geom1_list, trans_list)]
        ...
        for geom1, geom2_list, attr2_dict, trans in zip(geom1_list, geom2_llist, attr2_dlist, trans_list):
            poly2_intersect_list, bool_list = geo_geom.find_intersect_polygons(
                geom1, geom2_list)
            geom2_sub_list = [geom for (geom, bool) in zip(
                geom2_list, bool_list) if bool]
            attr2_sub_dict = {}
            for key in keys:
                attr2_sub_dict[key] = [attr for (attr, bool) in zip(
                    attr2_dict[key], bool_list) if bool]
    ...
    def export(self, infile, option=None):
        ...
        if self.output_type == "csv_list_line":
            geom = geo_geom.create_lineString(
                [latlon[::sign] for latlon in self.output])
            geo_io.make_vector_from_geom(geom, None, outfile=infile)

        elif self.output_type == "csv_list_polygon":
            geom = geo_geom.create_polygon(
                [latlon[::sign] for latlon in self.output])
            geo_io.make_vector_from_geom(geom, None, outfile=infile)

        elif self.output_type == "csv_list_polygons":
            geom_list = geo_geom.create_polygons(
                [[latlon[::sign] for latlon in polygon] for polygon in self.output])
            attr = {}
            attr["StartTime"] = [
                date_obj.strftime("%Y-%m-%d %H:%M:%S") for date_obj in self.output_attribute["StartTime"]
            ]
            attr["EndTime"] = [date_obj.strftime(
                "%Y-%m-%d %H:%M:%S") for date_obj in self.output_attribute["EndTime"]]
...
class SatelliteCatalog:
    ...
    def __init__(self, infile=True):
        if infile:
            infile = os.path.dirname(__file__) + os.sep + \
                "data/two_line_elements/satcat.csv"
    ...
    def __init__(self, sensor_name):
        self.params = import_config(
            config_path=os.path.dirname(
                __file__) + os.sep + "conf/" + sensor_name + "_params.yaml"
        )
        ...
        for geom_list, attr_dict in zip(geom_llist, attr_dlist):
            ...
            for i in range(len(geom_list) - 1):
                if attr2_list[i] == attr1_list[i + 1]:
                    ...
                    geom_list[i] = 0
            attr_dict["StartTime"] = [attr1 for (attr1, geom) in zip(
                attr1_list, geom_list) if geom != 0]
            attr_dict["EndTime"] = [attr2 for (attr2, geom) in zip(
                attr2_list, geom_list) if geom != 0]
