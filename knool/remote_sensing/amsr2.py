...
class AMSR2_L1R:
    ...
    def get_latlon(self, resolution="low"):
        ...
        lon = self.get_subds(self.subdsID["Longitude_of_Observation_Point_for_89A"]).ReadAsArray()[self.clip_array]
        if resolution == "low":
            lat = lat[:, 0::2]
            lon = lon[:, 0::2]
        return lat, lon
    ...
    def set_output_prop(self):
        ...
        for i in range(0, length, 10):
            ...
            gcps.append(gdal.GCP(float(lon[i][0]), float(lat[i][0]), 0.0, 0.5, i + 0.5))
            gcps.append(
