...
class MXD021KM():
    ...
    def _radiance_to_bt(self,radiance,band_name):
        ...
        a=h*c/(k*lamda); b=2*h*c**2/lamda**5
    ...
    def get_calibrated_em(self,band_min=0,band_max=10000):    
        ...
        bt=np.array([self._radiance_to_bt(((inArray[band,:,:]-offsets[band])*scales[band])*1.0E6,self.get_em_bnames()[band]) for band in range(band_min,band_max)])
        self.output=bt
