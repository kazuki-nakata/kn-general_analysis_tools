import sys
import os
from knool.geodata_processor.geo_table import geom_shpl_from_ogr
import numpy as np
import pandas as pd
...
class TLEsPerSat():
    ...
    def calc_intersect_scene_areas(self,fdate,ldate,interval,distance,pinterval,geom1_list0,trans_list,ori="middle"):
        geom1_list=[]
        ...
        for geom,trans in zip(geom1_list,trans_list):
            ...
            bool_list=np.max(geo_info.calc_distances(self.output[:,:,1],self.output[:,:,0],lon,lat),axis=1)<5000
    ...
    def export(self,infile,option=None):
        ...
        if self.output_type=="csv_list_line":
            ...
            attr={}
            attr["StartTime"]=[date_obj.strftime('%Y-%m-%d %H:%M:%S') for date_obj in self.output_attribute["StartTime"]]
            ...
            attr={}
            attr["StartTime"]=[date_obj.strftime('%Y-%m-%d %H:%M:%S') for date_obj in self.output_attribute[index]["StartTime"]]
            attr["EndTime"]=[date_obj.strftime('%Y-%m-%d %H:%M:%S') for date_obj in self.output_attribute[index]["EndTime"]]
            if sign==-1: [geom.SwapXY() for geom in geom_list]
            geo_io.export_vector_from_geomList(infile,geom_list,attr)

class SatelliteCatalog():

    ITEM_NAME=["OBJECT_NAME","OBJECT_ID","NORAD_CAT_ID","OBJECT_TYPE","OPS_STATUS_CODE",
    "OWNER","LAUNCH_DATE","LAUNCH_SITE","DECAY_DATE","PERIOD","INCLINATION","APOGEE",
...
class Sensor():
    def __init__(self,sensor_name):
        self.params=import_config(config_path=os.path.dirname(__file__) + os.sep + 'conf/'+sensor_name+'_params.yaml') 
        self.param_keys=self.params.keys()
        satcat=SatelliteCatalog()
        self.ncid=satcat.get_ncid_from_name(self.params["satellite"])
        tle_path=os.path.dirname(__file__) + os.sep + 'data'+os.sep+"two_line_elements"+os.sep+"sat"+str(self.ncid)+'.txt'
    ...
    def calc_intersect_scene_areas(self,stime,etime,interval,pinterval,aoi_list,trans_list,scene_all=False):
        ...
        obs_offset=self.params["obs_offset"]
        result1, result2 = self.sat.calc_intersect_scene_areas(stime,etime,interval,obs_width,pinterval,aoi_list,trans_list,ori)
        if scene_all:
            result=result2
