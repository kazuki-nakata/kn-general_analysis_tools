...
SUBROUTINE rSIR(grid_x,grid_y,vs,vb,vg,time,val,mask_grid,lon_grid,lat_grid,out_val,out_time,n_grid,nx,ny,&
day,ltod_min,ltod_max,rot_offset,wsize,ap,nx_ap,ny_ap,int_ap,res,fwhm)
!lat_grid,lon_grid -> for getting local time of day
...
INTEGER(4),INTENT(IN) :: n_grid,nx,ny,day,wsize,nx_ap,ny_ap
! INTEGER(4),INTENT(IN) :: i_t,j_t
REAL(4),INTENT(IN) :: rot_offset,ltod_min,ltod_max,int_ap,res,fwhm
REAL(4),INTENT(IN),DIMENSION(1:nx,1:ny) :: lon_grid, lat_grid
...
do l = 1, n_grid
    ...
    lon2 = lon_grid(i,j) - rot_offset
    if(lon2.gt.180) lon2=lon2-360.
    if(lon2.lt.-180) lon2=lon2+360
    ltod=time(l)+lon2/360.-real(day-1)
    if((ltod.ge.ltod_min-rot_offset2).and.(ltod.le.ltod_max-rot_offset2)) then
      pindex(l)=findex(i,j)
      uindex(l)=1
      findex(i,j)=l
    endif
...
SUBROUTINE test_SIR(grid_x,grid_y,val,mask_grid,out_val,n_grid,nx,ny,wsize,ap,nx_ap,ny_ap,int_ap,res,fwhm)
IMPLICIT NONE
INTEGER :: i,ii,j,jj,k,kk,l,nz,grid_i,grid_j,isum
INTEGER :: i2,j2,i3,j3,p,dum,ip,jp
...
END SUBROUTINE test_SIR



SUBROUTINE L1B_to_L3_WA(grid_x,grid_y,time,val,mask_grid,lon_grid,lat_grid,out_val,out_time,&
n_grid,nx,ny,day,ltod_min,ltod_max,rot_offset,wsize,fwhm,res)
!lat,lon -> for getting local time of day
!WA: weighted averaging
IMPLICIT NONE
INTEGER :: i,ii,j,jj,k,kk,l,nz,grid_i,grid_j,isum
INTEGER :: i2,j2,i3,j3,p,dum
INTEGER(4),INTENT(IN) :: n_grid,nx,ny,day,wsize
REAL(4),INTENT(IN) :: rot_offset,ltod_min,ltod_max,res,fwhm
INTEGER(4),INTENT(IN) :: mask_grid(1:nx,1:ny)
REAL(4),INTENT(IN) :: lon_grid(1:nx,1:ny), lat_grid(1:nx,1:ny)
