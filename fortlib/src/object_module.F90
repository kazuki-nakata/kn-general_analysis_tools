...
CONTAINS
  ...
  SUBROUTINE init_unionfind(width,length,data,n_prop,mask,num)
    ...
    REAL(4),INTENT(IN) :: data(1:width,1:length,n_prop)
...
!------------------------------------------------
    ...
    ALLOCATE(djsdata(1:ptr))
    ALLOCATE(sllptr(1:ptr))
    ALLOCATE(dllptr(1:ptr))
...
!   djsdata(i1)%property(2)=(s1*djsdata(ph1)%property(2)+s2*djsdata(ph2)%property(2))/(s1+s2)
   djsdata(i2)%size=0
