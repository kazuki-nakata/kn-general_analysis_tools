...
SUBROUTINE bilin_down_scaling_real_with_mask(input,output,mask0,land,isize,jsize,isize2,jsize2)
   ...
   IMPLICIT NONE
   INTEGER :: i,j,k,i2,j2
   REAL(4) :: var(4),dx(4),dy(4),mask(4)
   INTEGER,INTENT(IN) :: isize,jsize,isize2,jsize2,mask0
   REAL(4),INTENT(IN) :: input(isize,jsize)
   REAL(4)::input2(0:isize+1,0:jsize+1)
   INTEGER(4),INTENT(IN) :: land(isize2,jsize2)
   REAL(4),INTENT(OUT):: output(1:isize2,1:jsize2)
   input2(1:isize2,1:jsize2)=input
   input2(0,:)=mask0;input2(:,0)=mask0
   input2(isize+1,:)=mask0;input2(:,jsize+1)=mask0
   output(:,:)=9.9E33
   do j =1,jsize2
      do i=1,isize2
         ...
         var(1)=input2(int(i/2),int(j/2))
         var(2)=input2(int((i+2)/2),int((j+2)/2))
         var(3)=input2(int((i+2)/2),int(j/2))
         var(4)=input2(int(i/2),int((j+2)/2))
         mask(:)=0
         if(var(1).eq.mask0) mask(1)=999
         if(var(2).eq.mask0) mask(2)=999
         if(var(3).eq.mask0) mask(3)=999
         if(var(4).eq.mask0) mask(4)=999
         dx(1)=abs(real(int(i/2)*2)-0.5-real(i))/2
         dy(1)=abs(real(int(j/2)*2)-0.5-real(j))/2
         dx(2)=abs(real(int((i+2)/2)*2)-0.5-real(i))/2
         dy(2)=abs(real(int((j+2)/2)*2)-0.5-real(j))/2
         dx(3)=abs(real(int((i+2)/2)*2)-0.5-real(i))/2
         dy(3)=abs(real(int(j/2)*2)-0.5-real(j))/2
         dx(4)=abs(real(int(i/2)*2)-0.5-real(i))/2
         dy(4)=abs(real(int((j+2)/2)*2)-0.5-real(j))/2
         output(i,j)=bilin(dx,dy,var,mask,int(4))
      enddo
   enddo

END SUBROUTINE bilin_down_scaling_real_with_mask
...
function bilin(dx,dy,var,mask,isize)
  ...
  REAL(4),INTENT(in) :: var(isize),dx(isize),dy(isize),mask(isize)
  REAL(4) :: num1,num2
  INTEGER(4) :: i
