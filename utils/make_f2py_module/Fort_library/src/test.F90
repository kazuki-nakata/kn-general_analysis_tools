...
!           print *,data_i(k,j,i)
        ...
        argmax_prob1 = 0
        argmax_prob2 = 0
        param_a = dble(prior_beta(1,j,i))
        param_b = dble(prior_beta(2,j,i))
...
!           print *,sum,t,m,n,param_a,param_b
           ...
           m=dble(sum)+param_a
           n=dble(inum)-dble(t)-dble(sum)+param_b
!           print *,m,n
...
!           if(prob_t.ge.max_prob_t) then
 ...
 !       if(max_prob_t .ge. REAL(8) FUNCTION gamma_d(a)
  implicit none
  REAL(8),intent(in) :: a
  INTEGER :: k
  INTEGER :: i,j
...
end FUNCTION gamma_d0.5) openland=1
